...
I'm most comfortable in Python, including Flask APIs, debugging existing
codebases, and working with LLM-integration code (I've built several
projects wiring the Groq API into classification and RAG pipelines, plus
ChromaDB and sentence-embedding-based retrieval). My research background is
in deep learning for healthcare imaging and time series, so I'm also
comfortable reading and modifying PyTorch/model-training code, though most
of that work has been research scripts rather than production-style
software with test suites and CI.

I want to get better at: working inside a larger, unfamiliar codebase
rather than a project I built from scratch, and at the conventions of a
real open-source review process (PR etiquette, responding to maintainer
feedback, CI green-lighting).

I'd like to avoid: issues that are purely frontend/UI-styling work with no
backend or logic component, and issues in languages I haven't worked in
before (I'd rather stretch on process and codebase size this first round,
not on an unfamiliar language at the same time).
